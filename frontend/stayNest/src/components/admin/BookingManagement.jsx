...
const BookingManagement = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/admin/bookings?${params}`);
  ...
  const updateBookingStatus = async (bookingId, status, adminNotes = '') => {
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/admin/bookings/${bookingId}/status`, { 
        status, 
        adminNotes 
      });
  ...
  const processRefund = async (bookingId, amount, reason) => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/admin/bookings/${bookingId}/refund`, {
  ...
  const handleRefund = (booking) => {
    ...
    if (amount && parseFloat(amount) > 0 && parseFloat(amount) <= booking.totalPrice) {
                ...
                {booking.refund && (
                  <div className="mt-4 p-3 bg-purple-50 border border-purple-200 rounded">
                    <p className="text-sm text-purple-800">
                      <strong>Refund:</strong> ${booking.refund.amount} - {booking.refund.reason}
                    </p>
                    <p className="text-xs text-purple-600">
                      Processed on {new Date(booking.refund.processedAt).toLocaleDateString()}
                    </p>
                  </div>
                )}
              </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateBookingStatus(booking._id, e.target.value, notes || '');
                    }}
